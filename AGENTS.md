...
2. Add the catalogue entry in the same PR: stable `id`, exact `file` path, title, description, model, output, tags, `status: "published"`, `publishedAt` (UTC ISO date), difficulty (`Beginner` or `Advanced`), and nonempty `inputs`/`outputs` arrays (`Text`, `Image`, `Video`, `Audio`, `Reference pack`). Notes are optional. `body` is optional Markdown shown on the workflow's website page (headings, lists, bold, links; no HTML tags; under 20,000 characters); `description` stays the one-line summary. Cover images are not accepted by the platform importer yet; omit `coverImage`. Do not include credentials, personal paths or private prompts.
...
Local validation: run `npx --yes --package=https://codeload.github.com/Hearmeman24/comfyui-qwen-template/tar.gz/066a32758d4f883bc068656c27fd3a486649eacd catalog-check --root . --base-ref "$(git rev-parse origin/main)"`. CI uses this exact pinned contract.
