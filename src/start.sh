#!/usr/bin/env bash
# Version guard for pre-migration images (v9 and earlier). NOT the boot script.
#
...
# hard-resets the clone to origin/main on every boot, copies /start.sh out of
# it, and runs `bash /start.sh`). The migrated entrypoint execs
...
# src/hf_download_manager.py and src/build_manifest.py (both deleted by the
# migration) emits two "cp: cannot stat" lines to stderr. They run under
# `set +e`, so they are non-fatal, and the banner below dominates the log.
# Do not add stub .py files to suppress them.
...
============================================================
  ...
  To get running again:
    ...
    1. Edit your RunPod template and set the image to:
         hearmeman/comfyui-ltx-template:v10
